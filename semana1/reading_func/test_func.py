...
def test_convert_to_fahrenheit():
    ...
    assert convert_to_fahrenheit(reading_hum) == 50.0 # Como es de humedad no deberia alterar el valor

# Importante, como en func.py ya devuelve un valor bool, no debemos poner == True/False, es una mala practica
...
def test_apply_calibration_offset():
    ...
    assert cal_reading.sensor_type == reading.sensor_type # El tipo de sensor deberia permanecer igual
...
def test_format_alert_message():
    ...
    expected_message = "Alerta: El sensor_1 (TEMPERATURE) reporto un valor inesperado: 100.00. Razon: El valor excedio el umbral"
