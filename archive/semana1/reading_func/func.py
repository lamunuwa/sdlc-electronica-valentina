...
def apply_calibration_offset(r: Reading, offset: float) -> Reading:  # Calibracion
    return Reading(
        ...
        sensor_type=r.sensor_type,  # Usamos frozen para crear una cota sin modificar la original
    )


def format_alert_message(r: Reading, reason: str) -> str:  # Notificaciones
    return f"Alerta: El {r.sensor_id} ({r.sensor_type.name}) reporto un valor inesperado: {r.value:.2f}. Razon: {reason}"  # noqa: E501
